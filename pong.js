...
Computer.prototype.update = function(ball) {
  ...
  if(diff < 0 && diff > -2) { // max speed left
    diff = -5;
  } else if(diff > 0 && diff > 2) { // max speed right
    diff = 5;
  }
...
Ball.prototype.update = function(paddle1, paddle2) {
  ...
  if(this.x - 5 < 0) { // hitting the left wall
    this.x = 5;
    this.x_speed = -this.x_speed // the bounce off the wall
  ...
  if(this.y < 0 || this.y > 600) { // a point was scored
    this.x_speed=0;
    this.y_speed=3;  // turn to elsif statement and add a score variable
    this.x = 200;
    this.y = 300;
  }
